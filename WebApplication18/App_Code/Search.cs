...
public class DropDownListHandler
{
    ...
    private static List<DataStandard> _DataSetUpStandard;
    ...
    // Load Data From PFC
    public Dictionary<string, List<object>> LoadDataPara(string textSearch, bool searchByID)
    {
        Dictionary<string, List<object>> packageData = new Dictionary<string, List<object>>();

        using (SqlConnection con = new SqlConnection(_connectionString))
        {
            string query = "SELECT * FROM dbo.PFCTable ORDER BY TagName ASC";

            if (searchByID)
            {
                query = "SELECT * FROM dbo.PFCTable WHERE EXISTS (SELECT 1 FROM dbo.AlarmTable WHERE ID = @TextSearch AND TagDesc = PFCTable.TagDesc);";
            }
            else if (textSearch != "All") // Search by TagDesc (En-Us)
            {
                query = "SELECT * FROM dbo.PFCTable WHERE TagDesc = @TextSearch";
            }
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        string PositionMachine = reader["FactoryArea"] as string + "-" + reader["Location"] as string;
    ...
    public List<object> LoadStatusdisable(string DatabaseName, string[] KeyName, string[] KeyCondition, string[] ValueCondition)
    {

        return LoadoutDatabase("dbo.PFCTable", KeyName, KeyCondition, ValueCondition); 
    }

...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
                        ...
                        <div class="modal-Infor1">
                            ...
                            <div class="form-group">
                                ...
                                <select id="dropdown_Modal" class="form-control">
                                    ...
                                    <option value="Transmission error">
                                        <asp:Label runat="server" Text="Error5_confirm" meta:resourceKey="Error5_confirm"></asp:Label></option>
    ...
    <script>
        ...
        function updateTable(data, NameTable) {
            ...
            $.each(data, function (index, item) {
                ...
                icon.on('click', function (e) {
                    ...
                    // Create a string with all the information in the row
                    var info = new Array(cells.length);
                    for (var i = 0; i < cells.length; i++) {
                        info[i] = $(cells[i]).text();
                    }

                    // update value to Popup

                    document.getElementById('ID_Modal').textContent = info[0];
                    document.getElementById('Description_Modal').textContent = info[1];
                    document.getElementById('Value_Modal').textContent = info[3];
                    document.getElementById('Line_Modal').textContent = info[2];
                    var dropdown = document.getElementById('dropdown_Modal');
                    for (var i = 0; i < dropdown.options.length; i++) {
                        if (dropdown.options[i].value === info[8]) {
                            dropdown.selectedIndex = i;
                            break;
                        }
                    }
                    document.getElementById('Text_Modal').value = info[9];
        ...
        function handleParameterUpdate() {
            ...
            adjustParameters(keyLabel);
        }

        // Adjust parameters based on key label
        function adjustParameters(keyLabel) {
        ...
        function updateParameterLevel(ID_Selected, _SearchByID_Number) {
            return new Promise((resolve, reject) => {
                $.ajax({
                    ...
                    data: JSON.stringify({ ID: ID_Selected, SearchByID: _SearchByID_Number }),
                    contentType: "application/json; charset=UTF-8",
                    dataType: "json",
                    success: function (Data_Receive) {
        ...
        function updateTimestamp(minusminute, target) {
        ...
        function CreatetagPFC() {
            ...
            $.ajax({
                type: "POST",
                url: "Alarm.aspx/LoadDataPara",       
                data: JSON.stringify({ ID: "All", SearchByID : false }),
                contentType: "application/json; charset=UTF-8",
                dataType: "json",
                success: function (Data_TagDesc_Receive) {
                    Package_DataPara = JSON.parse(Data_TagDesc_Receive.d);
                ...
                if (confirmChange) {
                    ...
                    DisableAlarm(IDEnable.id, value, null, function (isSuccess) {
                        ...
                        if (isSuccess) {
                            ...
                            LoadDisableAlarm(Parameters[0], Parameters[1], Parameters[2]); 
                        } else {
            ...
            values.forEach(function (value) {
               ...
               /* textSpan.style.display = 'none'; // Hide the span*/
                ...
                listItem.onclick = function (event) {
                    ...
                    ModifyLevel(this.id, textContent);
                };
                ...
                if (!machineContainers[machineID]) {
                    ...
                    element.id = value.MachineType + "-" + value.FactoryArea + "-" + value.Location; 
                    optionsList.setAttribute('id', value.MachineType + "-" + value.FactoryArea + "-" + value.Location);
                    ButtonEnable.id = value.MachineType + "-" + value.FactoryArea + "-" + value.Location +"_Button";
                    ...
                    ButtonEnableMachineId.addEventListener('click', function (event) {
                        ...
                        if (confirmChange) {
                            ...
                            DisableAlarm(optionsList.id, FlagValue, MachineID.id, function (isSuccess) {
                                ...
                                if (isSuccess) {
                                    LoadDisableAlarm(value.MachineType, value.FactoryArea, value.Location); 
        ...
        function LoadDisableAlarm(Machine, FactoryArea, Location ){
            ...
            $.ajax({
                ...
                success: function (Data_Receive) {
                    ...
                    Data_Receive.forEach(function (value) {
                        ...
                        var MachineTypeButton = document.getElementById(parentOptions.id + "_Button");
    ...
    <script>
        ...
        function ModifyLevel(iconId, TextContext) {
        ...
        function adjustDate(calendarInput, adjustment, EnableAjustDay) {
            ...
            if (sessionStorage.getItem('_LoadCurrentTime') === "true") {
                ...
                updateTimestamp(scaleMultiplier, calendarInput);
        ...
        function RemoveSelectedInColorBar() {
            ...
            console.log(sessionStorage.getItem('_AjustThreshold_ID'));

    ...
    public partial class Alarm : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                PFCLable LableForLevel = new PFCLable();
        ...
        public static void UpdateAlarmStatus(Dictionary<string, object>  packages, bool isChangePFC )
        {
            ...
            if (!isChangePFC) // Check if user set up parameters
            {
                ...
                string[] keyConditionPFCTable = new string[] { "TagDesc" };
                string[] valueByConditionPFCTable = { packages["TagDesc"].ToString() };

                Hand.UpdateInDatabase("dbo.PFCTable", keyNamePFCTable, valueByKeyPFCTable, keyConditionPFCTable, valueByConditionPFCTable);
            }
        ...
        [WebMethod]
        public static string LoadDataPara(string ID, bool SearchByID)
        {
            try
            {
                languages = HttpContext.Current.Request.UrlReferrer.Query.Split('=')[1] as string;
            ...
            PackageData = Hand.LoadDataPara(ID, SearchByID);
        ...
        public static bool DisableAlarm(Dictionary<string, object>  Package)
            ...
            if (string.IsNullOrWhiteSpace(Package["MachineID"] as string))
            {
                ...
                keyConditionPFCTable = new string[] { "Machine", "FactoryArea", "Location" };
                valueByConditionPFCTable = new string[] { Package["Machine"].ToString(), Package["FactoryArea"].ToString(), Package["Location"].ToString() };
            }
            else
            {

                keyConditionPFCTable = new string[] { "Machine", "FactoryArea", "Location", "MachineID" };
                valueByConditionPFCTable = new string[] { Package["Machine"].ToString(), Package["FactoryArea"].ToString(), Package["Location"].ToString(), Package["MachineID"].ToString() };
            }
            try
            {
                Hand.UpdateInDatabase("dbo.PFCTable", keyNamePFCTable, valueByKeyPFCTable, keyConditionPFCTable, valueByConditionPFCTable);
        ...
        public static string LoadstatusDisableAlarm(Dictionary<string, object> Package)
        {
            ...
            string[] keyConditionPFCTable, valueByConditionPFCTable;
            string[] keyNamePFCTable = new string[] { "MachineID", "EnableAlarm" , "Machine" };
            ...
            List<object> result = Hand.LoadStatusdisable("dbo.PFCTable", keyNamePFCTable, keyConditionPFCTable, valueByConditionPFCTable);
